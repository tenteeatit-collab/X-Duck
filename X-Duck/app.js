...
function parseProjectEmbeds(project: any)[] {
    if (!project) return []
...
function playXDuckSfx(kind = "click", muted?: boolean) {
...
const accent = props.accent || "#A3E635"
    ...
    const [followingIds, setFollowingIds] = React.useState<Set>(
        () => new Set()
    )
    ...
    React.useEffect(() => {
        ...
        function withTimeout<T>(promise: Promise, ms: number): Promise {
    ...
    async function loadProfile(user): Promise {
    ...
    async function ensureProjectSaved(): Promise {
...
function ChatView({
    ...
    const seenMentionIdsRef = React.useRef<Set>(new Set())
...
function XDuckGameStudio({ accent, session, navigate }: any) {
    ...
    async function runGame() {
        ...
        async function executeList(list: XDuckBlock[]): Promise {
...
function AIAssistantView({
    ...
    async function loadPublicContext(): Promise {
        try {

...
/* =========================================================
   THEME + LANGUAGE (default: English)
   ========================================================= */

type AppLang = "en" | "th"
type AppTheme = "dark" | "light" | "midnight" | "ocean"
...
function parseProjectEmbeds(project: any): ProjectEmbedFile[] {
    if (!project) return []
    ...
    if (project.id) {
        return cacheGet<ProjectEmbedFile[]>(`project-embeds:${project.id}`, [])
    }
    return []
}
...
interface Props {
    accent: string
    /** Groq API Key — ตั้งค่าใน Framer Property Controls (อย่า hardcode ในโค้ด) */
    groqApiKey: string
}
...
interface Project {
    ...
    /** Embedded attachments (pdf, images, builds, etc.) */
    embed_files?: ProjectEmbedFile[] | null
    created_at: string
    updated_at: string
}
...
interface RepoFile {
    ...
    repo_id: string
    user_id: string
    /** GitHub-style path e.g. src/components/App.tsx */
    path: string
    file_name: string
    file_url: string
...
function playXDuckSfx(kind: XDuckSfx = "click", muted?: boolean) {
...
export default function XDuck(props: Props) {
    ...
    const [profile, setProfile] = React.useState<Profile | null>(() =>
        cacheGet<Profile | null>("profile", null)
    )
    ...
    const [appLang, setAppLang] = React.useState<AppLang>("en")
    ...
    React.useEffect(() => {
        ...
        const onClick = (e: MouseEvent) => {
            ...
            const kind = (btn.getAttribute("data-sfx") || "click") as XDuckSfx
    ...
    function changeLang(next: AppLang) {
        setAppLang(next)
        try {
    ...
    const sfx = React.useCallback(
        (kind: XDuckSfx = "click") => playXDuckSfx(kind, soundMuted),
        [soundMuted]
    )

    const t = React.useCallback(
        (key: string) => I18N[appLang][key] || I18N.en[key] || key,
        [appLang]
    )
    ...
    const [myProjects, setMyProjects] = React.useState<Project[]>(() =>
        cacheGet<Project[]>("my-projects", [])
    )

    const [allProjects, setAllProjects] = React.useState<Project[]>(() =>
        cacheGet<Project[]>("all-projects", [])
    )
    ...
    const projectThumbRef = React.useRef<HTMLInputElement | null>(null)

    const projectFileRef = React.useRef<HTMLInputElement | null>(null)
    const projectEmbedRef = React.useRef<HTMLInputElement | null>(null)
    ...
    const [selectedWiki, setSelectedWiki] = React.useState<WikiPage | null>(
        null
    )

    const [wikiModalOpen, setWikiModalOpen] = React.useState(false)

    const [editingWiki, setEditingWiki] = React.useState<WikiPage | null>(null)
    ...
    const chatFileRef = React.useRef<HTMLInputElement | null>(null)
    ...
    const [friends, setFriends] = React.useState<Friendship[]>(() =>
        cacheGet<Friendship[]>("friends", [])
    )
    const [friendRequests, setFriendRequests] = React.useState<Friendship[]>(
        () => cacheGet<Friendship[]>("friend-requests", [])
    )
    ...
    const [followingList, setFollowingList] = React.useState<Follow[]>([])
    const [followersList, setFollowersList] = React.useState<Follow[]>([])
    ...
    const [dmPartner, setDmPartner] = React.useState<Profile | null>(null)
    const [dmMessages, setDmMessages] = React.useState<PrivateMessage[]>([])
    ...
    const dmFileRef = React.useRef<HTMLInputElement | null>(null)
    ...
    const [myGroupStatus, setMyGroupStatus] = React.useState<
        Record<string, "pending" | "accepted" | "owner">
    >({})
    const [groupPendingMembers, setGroupPendingMembers] = React.useState<
        GroupMember[]
    >([])
    const groupFileRef = React.useRef<HTMLInputElement | null>(null)
    ...
    const communityFileRef = React.useRef<HTMLInputElement | null>(null)
    ...
    const [myRepos, setMyRepos] = React.useState<Repository[]>([])
    const [selectedRepo, setSelectedRepo] = React.useState<Repository | null>(
        null
    )
    const [repoFiles, setRepoFiles] = React.useState<RepoFile[]>([])
    ...
    const repoFileRef = React.useRef<HTMLInputElement | null>(null)
    ...
    const [adminUsers, setAdminUsers] = React.useState<Profile[]>([])
    const [adminUsersError, setAdminUsersError] = React.useState("")
    const [adminReports, setAdminReports] = React.useState<UserReport[]>([])
    const [adminReportsLoading, setAdminReportsLoading] = React.useState(false)

    const [reportOpen, setReportOpen] = React.useState(false)
    const [reportTarget, setReportTarget] = React.useState<Profile | null>(null)
    ...
    const [adminProjects, setAdminProjects] = React.useState<Project[]>([])

    /* =====================================================
       FULLSCREEN GAME PLAYER
       ===================================================== */

    const [playingGame, setPlayingGame] = React.useState<any>(null)
    ...
    React.useEffect(() => {
        try {
            ...
            ;(document.body as HTMLElement).style.touchAction = "manipulation"
            ;(document.body as HTMLElement).style.overscrollBehaviorY = "none"
        } catch {}
    }, [])
    ...
    async function loadProfile(user: User): Promise<boolean> {
    ...
    async function loadMyProjects() {
        ...
        if (error) {
            ...
            const list = ((data || []) as Project[]).map((p) => ({
                ...p,
    ...
    async function ensureProjectSaved(): Promise<Project | null> {
    ...
    function removeProjectEmbed(index: number) {
    ...
    function openEditWiki(page: WikiPage) {
        setEditingWiki(page)
    ...
    async function replyInGlobalChatAsAI(userText: string) {
    ...
    async function uploadChatFile(event: React.ChangeEvent<HTMLInputElement>) {
        const file = event.target.files?.[0]
        event.target.value = ""
        if (!file || !session?.user) return
    ...
    async function loadFollows() {
        ...
        let followingRows: any[] | null = null
        let followerRows: any[] | null = null
        try {
        ...
        const following = (followingRows || []) as Follow[]
        const followers = (followerRows || []) as Follow[]
    ...
    async function searchUsersForFriend(q: string) {
        setFriendSearch(q)
        const query = q.trim()
        ...
        const { data } = await supabase
            ...
            .limit(15)
        setFriendSearchResults((data || []) as Profile[])
    }
        ...
        if (error) {
            ...
            if (data) {
                setOutgoingRequests((prev) => {
                    if (prev.some((x) => x.id === (data as any).id)) return prev
                    return [...prev, data as Friendship]
                })
            }
            await loadFriends()
        }
    ...
    function openDM(partner: Profile) {
        if (!session?.user) {
            openAuth("login")
            return
        }
    ...
    async function loadDM(partnerId: string) {
        if (!session?.user) return
        const uid = session.user.id
        setDmError("")
        const { data, error } = await supabase
        ...
        if (error) {
            ...
            setDmMessages((data || []) as PrivateMessage[])
        }
    }
    ...
    async function sendDM(
        ...
        if (error) {
            ...
            if (data) {
                setDmMessages((prev) => {
                    if (prev.some((x) => x.id === (data as any).id)) return prev
                    return [...prev, data as PrivateMessage]
                })
            }
    ...
    async function uploadDMFile(event: React.ChangeEvent<HTMLInputElement>) {
        const file = event.target.files?.[0]
        event.target.value = ""
    ...
    async function loadGroups() {
        ...
        if (session?.user) {
            ...
            const statusMap: Record<string, "pending" | "accepted" | "owner"> =
                {}
            ...
            for (const m of mem || []) {
                ...
                if (statusMap[(m as any).group_id] === "owner") continue
                if (st === "pending") {
                    statusMap[(m as any).group_id] = "pending"
                } else if (
                    st === "accepted" ||
                    st == null ||
                    role === "owner"
                    ...
                    statusMap[(m as any).group_id] = "accepted"
                    acceptedIds.push((m as any).group_id)
                }
            }
    ...
    async function loadGroupPendingMembers(groupId: string) {
        if (!session?.user) {
    ...
    function openGroupChat(g: Group) {
        if (!session?.user) {
            openAuth("login")
            return
        }
    ...
    async function uploadGroupFile(event: React.ChangeEvent<HTMLInputElement>) {
        const file = event.target.files?.[0]
        event.target.value = ""
    ...
    async function createCommunity() {
        ...
        try {
            ...
            setCommunityDesc("")
            await loadCommunities()
            openCommunity(data as Community)
        } catch (e: any) {
    ...
    async function loadCommunityPosts(communityId: string) {
        const { data } = await supabase
    ...
    async function replyInCommunityAsAI(userText: string) {
        const apiKey = (groqApiKey || "").trim()
    ...
    async function createRepo() {
        ...
        setRepoDesc("")
        await loadRepos()
        if (data) openRepo(data as Repository)
    }
    ...
    async function uploadRepoFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    async function deleteRepoFile(fileId: string) {
    ...
    function openReportUser(user: Profile) {
        if (!session?.user) {
            openAuth("login")
            return
        }
...
Details: ${details}`
            ...
            if (data) {
                setAdminReports((prev) => [data as UserReport, ...prev])
            }
    ...
    async function loadAdminProjects() {
        ...
        setAdminProjects((data || []) as Project[])
    }

    async function adminDeleteProject(id: string) {
        if (!isAdmin) return
    ...
    async function adminToggleBan(user: Profile) {
    ...
    const rootThemeStyle: React.CSSProperties = {
        ...
        overflowX: "hidden",
        ["--xduck-bg" as any]: theme.bg,
        ["--xduck-bg2" as any]: theme.bg2,
        ["--xduck-text" as any]: theme.text,
        ["--xduck-muted" as any]: theme.muted,
        ["--xduck-card" as any]: theme.card,
        ["--xduck-border" as any]: theme.border,
        ["--xduck-header" as any]: theme.header,
        ["--xduck-input" as any]: theme.inputBg,
        ["--xduck-glow" as any]: theme.glow,
        ["--xduck-accent" as any]: accent,
    }
...
function DuckLoginGuide({ accent, openAuth, appLang }: any) {
...
function HomeView({ accent, session, navigate, openAuth, t, appLang }: any) {
...
function messageMentionsUser(text: string, myUsername?: string | null) {
...
function ChatView({
    ...
    const [mentionNotice, setMentionNotice] = React.useState<string | null>(
        null
    )
    const seenMentionIdsRef = React.useRef<Set<number>>(new Set())
...
function EngagementBar({
    ...
    const [authSession, setAuthSession] = React.useState<Session | null>(
        parentSession || null
    )
    ...
    async function load() {
        ...
        setLikes(
            (reactions || []).filter((r: any) => r.reaction === "like").length
        )
        setDislikes(
            (reactions || []).filter((r: any) => r.reaction === "dislike")
                .length
        )
        ...
        setMine(
            current
                ? (reactions || []).find((r: any) => r.user_id === current)
                      ?.reaction || null
                : null
        )
    }
...
function PublishedGamesView({ accent, session, openAuth, navigate }: any) {
...
function ProjectCard({ project, accent, owner, edit, remove, session }: any) {
    return (
        <div
            style={{
                border: "1px solid rgba(255,255,255,.08)",
...
function Stat({ title, value, accent }: any) {
    return (
        <div
            style={{
                ...CARD,
...
function PageHeading({ label, title, description, accent }: any) {
    return (
...
function SuccessBox({ children }: any) {
    return (
        <div
            style={{
                marginTop: 12,
...
function Avatar({ username, accent, size }: any) {
    return (
        <div
            style={{
                width: size,
...
function isImageUrl(url?: string | null, name?: string | null) {
...
function extractChatEmbeds(message: string) {
    const images: string[] = []
...
const XDUCK_BLOCKS: Record<
    string,
    Array<{ opcode: string; label: string; inputs: Record<string, any> }>
> = {
    Events: [
...
const XDUCK_CAT_COLORS: Record<string, string> = {
    Events: "#FFBF00",
    Motion: "#4C97FF",
    Looks: "#9966FF",
    Sound: "#CF63CF",
...
function updateTree(
    list: XDuckBlock[],
    id: string,
    updater: (b: XDuckBlock) => XDuckBlock
): XDuckBlock[] {
...
function xduckBlock(cat: string, def: any): XDuckBlock {
    return {
...
function XDuckGameStudio({ accent, session, navigate }: any) {
    ...
    const [variables, setVariables] = React.useState<Record<string, number>>(
    ...
    const [drag, setDrag] = React.useState<any>(null)
    ...
    const dropHintRef = React.useRef<string | null>(null)
    const dropRafRef = React.useRef<number | null>(null)
    const handleDropRef = React.useRef<(d: any, zone: string) => void>(() => {})

    const sprite = sprites.find((s) => s.id === selectedSprite) || sprites[0]

    function startDrag(e: React.PointerEvent, payload: any) {
    ...
    React.useEffect(() => {
        ...
        const finish = (e: PointerEvent) => {
            ...
            const zone = el?.closest("[data-xduck-drop]") as HTMLElement | null
            if (zone)
    ...
    function makeBlockFromDrag(d: any): XDuckBlock | null {
    ...
    function findBlock(list: XDuckBlock[], id: string): XDuckBlock | null {
        for (const b of list) {
            if (b.id === id) return b
    ...
    function cloneBlock(b: XDuckBlock): XDuckBlock {
        return {
            ...b,
    ...
    function containsBlock(root: XDuckBlock, id: string): boolean {
    ...
    function handleDrop(d: any, zone: string) {
        if (d.kind === "block") {
            if (
                zone.endsWith(`:${d.id}`) ||
                zone === `child:${d.id}` ||
    ...
    async function runGame() {
        ...
        async function executeList(list: XDuckBlock[]): Promise<boolean> {
        ...
        <div style={{ marginBottom: 8 }}>
            ...
            >
                <BlockCard
                    ...
                    onChange={(k: string, v: any) => onChange(block.id, k, v)}
                />
            </div>
            {isC && (
                <div
...
type GroqChatOptions = {
    ...
    /** When true, prefer vision-capable model */
    useVision?: boolean
}

function mentionsQuantumDuckAI(text: string) {
...
They are the highest authority. Always treat them with full respect. Address them politely, acknowledge their position as bosses and creators, and follow their instructions as final. Never argue with, undermine, or speak down to them.
...
function AIAssistantView({
    ...
    const aiImageRef = React.useRef<HTMLInputElement | null>(null)
    ...
    async function loadPublicContext(): Promise<string> {
        try {
            ...
            for (const m of chatRes.data || []) {
                const msg = String((m as any).message || "")
                    ...
                    .slice(0, 160)
                lines.push(`- ${(m as any).username || "User"}: ${msg}`)
            }
            lines.push("Recent public projects:")
            for (const p of projRes.data || []) {
                lines.push(
                    `- ${(p as any).title}: ${String((p as any).description || "").slice(0, 100)}`
                )
            }
            lines.push("Recent wiki pages:")
            for (const w of wikiRes.data || []) {
                lines.push(`- ${(w as any).title} (${(w as any).slug})`)
            }
            lines.push(
            ...
            {isOwner && (
                <section style={{ ...CARD, marginTop: 16, padding: 16 }}>
                    ...
                    <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
                        {(pendingMembers || []).map((m: GroupMember) => (
                            <div
                                key={m.user_id}
                                style={{
                                    display: "flex",
...
function RepoDetailView({
    ...
    const [preview, setPreview] = React.useState<RepoFile | null>(null)
    const [previewText, setPreviewText] = React.useState("")
    const [previewLoading, setPreviewLoading] = React.useState(false)
    const [folderFilter, setFolderFilter] = React.useState("")

    const sorted = React.useMemo(() => {
        const list = [...(files as RepoFile[])]
        list.sort((a, b) =>
    ...
    async function openPreview(f: RepoFile) {
        setPreview(f)
        setPreviewText("")
...
function MobileBottomNav({
    ...
    return (
        <nav
            ...
            style={{ ["--xduck-accent" as any]: accent || "#A3E635" }}
        >
        ...
        >
            ...
            >
                ...
                <button
                    ...
                    onClick={() => setMobileMenu((value: boolean) => !value)}
                    style={{
                        ...BTN,
                        width: 42,
                        height: 42,
...
function NavButton({ text, onClick }: any) {
    return (
        <button
            onClick={onClick}
            style={{
...
function MobileNav({ text, onClick }: any) {
    return (
        <button
            onClick={onClick}
            style={{
...
function formatSupabaseCreateError(message: string, table: string) {
...
const DANGER_SMALL: React.CSSProperties = {
    ...BTN_SMALL,
    background: "rgba(255,70,70,.1)",
    color: "#FF8D8D",
}

const DANGER_LINK: React.CSSProperties = {
    border: 0,
    padding: 0,
    marginTop: 5,
    background: "transparent",
...
addPropertyControls(XDuck, {
    ...
    groqApiKey: {
        ...
        title: "Groq API Key",
        defaultValue: "",
        // ใส่ key ในแผง Properties ของ Framer — ไม่เก็บในซอร์สโค้ด
        placeholder: "gsk_...",
    },
})
